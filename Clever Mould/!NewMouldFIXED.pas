...
begin
  ...
  Ph[mousey][mousex] := 1;
  Ph[mousey + 1][mousex] := 1;
  
  SetPixel(mousex, mousey, clYellow);
  SetPixel(mousex, mousey + 1, clYellow);
...
procedure DrawPh(x, y, pergr: integer);     //ðèñîâàíèå Ph
begin
    var sumofhod: integer; //ïðîâåðêà íà äîï õîäû
    ...
    if((y + 1 < SIZEY) and (y - 1 > 0) and (x + 1 < SIZEX) and (x - 1 > 0) and (SecondArr[y + 1][x] <> -1) and (Ph[y + 1][x] <> 1) and (Good[y + 1][x] < Good[y - 1][x]) and (Good[y + 1][x] < Good[y][x + 1]) and (Good[y + 1][x] < Good[y][x - 1]))then
    begin
    ...
    else if((y + 1 < SIZEY) and (y - 1 > 0) and (x + 1 < SIZEX) and (x - 1 > 0) and (SecondArr[y - 1][x] <> -1) and (Ph[y - 1][x] <> 1) and (Good[y - 1][x] < Good[y + 1][x]) and (Good[y - 1][x] < Good[y][x + 1]) and (Good[y - 1][x] < Good[y][x - 1]))then
    begin
        SetPixel(x, y - 1, clYellow);
        PhY[kolall + 1] := y - 1;
        PhX[kolall + 1] := x;
    ...
    else if((y + 1 < SIZEY) and (y - 1 > 0) and (x + 1 < SIZEX) and (x - 1 > 0) and (SecondArr[y][x + 1] <> -1) and (Ph[y][x + 1] <> 1) and (Good[y][x + 1] < Good[y - 1][x]) and (Good[y][x + 1] < Good[y][x - 1]) and (Good[y][x + 1] < Good[y + 1][x]))then
    begin
    ...
    begin
        ...
        per := random(1000);
        if((x + 1 < SIZEX) and (per >= 750) and (SecondArr[y][x + 1] <> -1) and (Ph[y][x + 1] <> 1))then
        begin
            SetPixel(x + 1, y, clYellow);
            PhY[kolall + 1] := y;
            PhX[kolall + 1] := x + 1;
        ...
        else if((x - 1 > 0) and (per >= 500) and (SecondArr[y][x - 1] <> -1) and (Ph[y][x - 1] <> 1))then
        begin
            SetPixel(x - 1, y, clYellow);
            PhY[kolall + 1] := y;
            PhX[kolall + 1] := x - 1;
        ...
        else if((y + 1 < SIZEY) and (per >= 250) and (SecondArr[y + 1][x] <> -1) and (Ph[y + 1][x] <> 1))then
        begin
            SetPixel(x, y + 1, clYellow);
            PhY[kolall + 1] := y + 1;
            PhX[kolall + 1] := x;
        ...
        else if((y - 1 > 0) and (SecondArr[y - 1][x] <> -1) and (Ph[y - 1][x] <> 1))then
        begin
            SetPixel(x, y - 1, clYellow);
            PhY[kolall + 1] := y - 1;
            PhX[kolall + 1] := x;
        ...
        begin
            var perr: integer;
            perr := random(3000);
            if((perr >= 2000) and (x - 1 > 0) and (SecondArr[y][x - 1] <> -1) and (Ph[y][x - 1] <> 1))then
            begin
                SetPixel(x - 1, y, clYellow);
                PhY[kolall + 1] := y;
                PhX[kolall + 1] := x - 1;
            ...
            begin
                perr := random(5000);
                if((perr >= 2500) and (y - 1 > 0) and (SecondArr[y - 1][x] <> -1) and (Ph[y - 1][x] <> 1))then
                begin
                    SetPixel(x, y - 1, clYellow);
                    PhY[kolall + 1] := y - 1;
                    PhX[kolall + 1] := x;
                ...
                else if((y + 1 < SIZEY) and (SecondArr[y + 1][x] <> -1) and (Ph[y + 1][x] <> 1))then
                begin
                    SetPixel(x, y + 1, clYellow);
                    PhY[kolall + 1] := y + 1;
                    PhX[kolall + 1] := x;
...
begin
    ...
    begin
        ...
        {if(z mod 500 = 0)then //óáèðàåò ìàëåíüêèå äûðû âíóòðè Ph
        begin
            ProvOfGr();
        end;}
        {if(z mod 1000 = 0)then   //óäàëåíèå
        begin
            var z2: integer;
            for z2:= 1 to 500 do
            begin
