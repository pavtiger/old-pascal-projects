...
var H, EarthX, EarthY, i, RingH, HeightObject, R, Count, Step, WhiteOrTranparent, 
  SaturnVHeight, StartButtonNumber, EndFlight, Gotcha, StarWars, S, W, ready: integer;
var X, Y, StartX, StartY, XTr, YTr, iTr, RTr, XSaturn1, YSaturn1, XSaturn2, YSaturn2, 
  ...
  HDownUp, ChangeH, TowerTriangle, StartFall, fall, Gotchai, StepFall, a, ra, StartFallH, StartFalli: real;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  {==}procedure LineFall(start, a: real);
  begin
    count:=0;
    var i: integer;
    for i := round(StartFalli) to round(StartFalli*2) do
    begin
      LineTo(XGetter(StartX, ra, i), YGetter(StartY, ra, i), clWhite);
      count+=1;
  ...
  {==}function SaturnLiftoffDraw(flight, falli: real):real;
  begin
    if flight=0 then
    begin
    ...
    begin
      StartFallH:=(((H+(H2-H1))/2)+flight+ChangeH);
      StartFalli:=(H/2+flight+DI/2);
      ready:=1;
    end;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  begin
    ...
    begin
      if EndFlight<>1 then
      begin
        StartLine:=SaturnLiftoffDraw(Fi, Gotchai);
        Fi+=StepRocket;
      end;
    end;
    TowerDraw(Fi);
