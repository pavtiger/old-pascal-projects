...
var X, Y, StartX, StartY, XTr, YTr, iTr, RTr, XSaturn1, YSaturn1, XSaturn2, YSaturn2, 
  ...
  HDownUp, ChangeH, TowerTriangle, Gotchai, StepFallH, StepTriangleFall, 
  SaturnRocketTriangle, FallCount, StartFall: real;
...
////////////////////////////////////////////////////////////////////////////////
  ...
  {==}function SaturnLiftoffDraw(flight, falli, StepHFall: real):real;
  begin
    if flight=0 then
    begin
    ...
    begin
      if (falli=0) or (FallCount>180) then
      begin
        StartFall:=((H+(H2-H1))/2)+flight;
        SaturnRocketTriangle-=StepTriangle;
      end
      else
      begin
        FallCount+=StepTriangleFall;
        SaturnRocketTriangle-=StepTriangle+StepTriangleFall;
      end;
    end;
    ...
    XRocket:=StartX + (((H+(H2-H1))/2{-falli})+flight+ChangeH)*cos((((H/2+flight+DI/2)*-1)+90)*Pi/180);
    YRocket:=StartY + (((H+(H2-H1))/2{-falli})+flight+ChangeH)*sin(((H/2+flight+DI/2)+90)*Pi/180);    
  ...
  {==}procedure LineDraw(start, falli: real);
  begin
    While true do
    begin
      Count+=1;
      ...
      begin
        if (RTr<StartFall) and (StartFall=0) then
        begin
          RTr+=-StepR;
          iTr-=-Stepi;
        end
...
////////////////////////////////////////////////////////////////////////////////
  ...
  begin
    ...
    begin
      if EndFlight<>1 then
      begin
        StartLine:=SaturnLiftoffDraw(Fi, Gotchai, StepRocket);
        if Gotcha<>1 then
        begin
          Fi+=StepRocket;
        end
    ...
    TowerDraw(Fi);
    i-=2;
    if (Gotcha=1) and (Gotchai<200) then
    begin
      Gotchai+=StepFallH;
    end;
