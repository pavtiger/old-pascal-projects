...
var Stars, Planet, EarthObject, SaturnV1, DeathStar, Tower, ResultStar, Vector: Picture;
var H, EarthX, EarthY, RingH, HeightObject, R, Count, Step, WhiteOrTranparent, 
  ...
  ReadyToFall, Test, NLevels, CaptureReady, StartPlay, SaturnVHeightNoFire, BadCapture, S, BreakDoAway, VectorH, define: integer;
var X, Y, StartX, StartY, XTr, YTr, iTr, RTr, XSaturn1, YSaturn1, XSaturn2, 
  ...
  TriangleR2, ExplodeH, DCapture, XSaturnHeightStart, YSaturnHeightStart, XSaturnHeightTower, YSaturnHeightTower, TowerMinus, SaturnVHeightTower, VectorTriangle: real;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    DeathStar := Picture.Create('Photoes\!Planets\'+ WayPlanets[2]);
    DeathStar.Load('Photoes\!Planets\'+ WayPlanets[2]);
    ...
    {Vector := Picture.Create('Photoes\!Rockets\'+ WayRockets[NLevels]);
    Vector.Load('Photoes\!Rockets\'+ WayRockets[NLevels]);
    {*}{Vector.Transparent := True;}
    
    Tower := Picture.Create('Photoes\Everything Else\Tower.bmp');
    Tower.Load('Photoes\Everything Else\Tower.bmp');
    {*}Tower.Transparent := True;
    
    ResultStar := Picture.Create('Photoes\Everything Else\StarResult.bmp');
    ResultStar.Load('Photoes\Everything Else\StarResult.bmp');
    {*}ResultStar.Transparent := True;
////////////////////////////////////////////////////////////////////////////////
    ...
    begin
      ...
      //if X<>0 then
      //begin
      //  Line(round(StartX), round(StartY), round(X), round(Y), clMediumVioletRed);
      //end;
      EarthObjectDraw(i);
      if StartButtonNumber=0 then
      begin
      ...
      //VectorTriangle:=Triangle(StartX, StartY, X, Y);
      //Vector.Draw(round(StartX), round(StartY), round(VectorTriangle), round(VectorH), round(VectorH));      
      if Starwars<>1 then
      begin
        EarthDraw();
      end
      ...
      begin
        if define=0 then
        begin
          EarthObject := Picture.Create('Photoes\!Objects\'+ WayObjects[2]);
          EarthObject.Load('Photoes\!Objects\'+ WayObjects[2]);
          {*}EarthObject.Transparent := True;
          define:=1;
        end;
        StarWarsDraw();
    ...
    //DXCapture:=abs(TriangleX2-X);
    //DYCapture:=abs(TriangleY2-Y);
    DCapture:=sqrt(sqr(DXCapture)+sqr(DYCapture));//abs(DXCapture+DYCapture);
    //print(DCapture);
    if DCapture>=CaptureH then
    begin
      DCapture:=CaptureH;
    end;
    if BreakDoAway=1 then
    begin
      BadCapture:=100;
    end
    else
    begin
      BadCapture:=round((100/(CaptureH))*DCapture);
      if (BadCapture=100) and (CaptureReady=1) then
      begin
        BadCapture:=99;
      end;
    end;
    TextOut(150, 500, 100-BadCapture+'%');
    redraw();
    mp3_LiftOff.Stop();
    
    for r:=1 to round((100-BadCapture)/33) do
    begin
      ResultStar.Draw(100+(r-1)*270, 90, 260, 260);
      redraw();
      sleep(1000);
    end;
